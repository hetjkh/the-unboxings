...
type CollectionImagePreloadProps = {
  /** LCP / above-the-fold image URLs to warm while the site loader is up. */
  urls: readonly string[];
  /** How many images must finish before we release the loader (keep low — LCP only). */
  waitFor?: number;
};

/**
 * Preloads only the LCP image(s) behind the loader (Image() only — no <link> in the
...
export default function CollectionImagePreload({
  ...
  const warmList = unique.slice(0, Math.max(waitFor, 1));
